...
if __name__ == '__main__':
    ...
    wood_path = args.analyse

    walls_color = np.array([252, 186, 3], dtype=np.uint8)
    # hsv = walls_color.reshape((1, 1, 3))
    # hsv = cv2.cvtColor(hsv, cv2.COLOR_RGB2HSV)[0, 0]
    # print(hsv)
    # black_coef = hsv[2]/255


    color1 = walls_color[::-1].tolist()
    ...
    #     mark_an_image(img_path, out)        wood_count = np.where
    main(img_path, out, wood_path, walls_color)
