...
def transport_shadows(img1, img2, black):
    img1 = Image.from_array(img1)
    img2 = Image.from_array(img2)
    img1.transform_colorspace('gray')
    img1.level(black=black, white=1, channel='all_channels')
    img2.composite_channel('all_channels', img1, 'soft_light', 0, 0)
    img2 = np.array(img2)
    return img2
...
def main(img_path, out,  wood_path, walls_color, draw_logo):
    ...
    # Wand wall color
    wand_wall_2change = transport_shadows(only_wall_orig, only_wall, -0.5)
    ...
    # Test
    wand_floor_2change = transport_shadows(original_floor_mask, img_output, -0.2)
...
def dump_info_to_json(json_path, walls_color, tile_identificator):
    ...
    info ={
        "walls_info": {
            ...
            "paint_logo": "logos/sample_walls_logo.png"
            },
        "floor_info": {
            "floor_title": "Suggested Flooring Material",
            "floor_name": floor_name,
            "floor_code": floor_code,
            "floor_icon": f"floor_thumbnails/sample_floor_icon_{tile_identificator}.png",
            "floor_logo": "logos/sample_floor_logo.png"
            }
        }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--analyse', type=str, nargs="+", required=True)
    ...
    img_path = args.img
    out = args.out
    args.analyse = ' '.join(args.analyse)
    args.analyse = args.analyse.replace(" ", "%20")
